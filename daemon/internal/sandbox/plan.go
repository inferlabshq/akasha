...
func (p Plan) Describe() string {
	...
	w("# secret in a shell startup file or a project env file that akasha has NOT")
	w("# discovered, nor one already exported into the environment akasha was")
	w("# launched from. A file a credential WAS discovered from is masked by")
	w("# provenance, whatever its name; the rest are the agent's working material,")
	w("# and masking them would break the run rather than protect it. Masking is")
	w("# therefore only ever as complete as discovery.")
	w("#")
