		...
		tool("vault_assume",
			...
			props(
				...
				opt("ttl_seconds", "number", "Seconds until the credential file is cleaned up (default 3600). "+
					"An agent is capped at 3600 regardless of what it asks for; longer work belongs in "+
					"`akasha run` or `akasha exec`, which supervise it. The response reports what was "+
					"actually granted in granted_ttl_seconds."),
			),
