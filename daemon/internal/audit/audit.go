...
// Tail returns the last n events from the active segment (rotated-out segments
// are not scanned — recent history lives in the active file). Simple enough for
// a local dev tool.
func (l *Logger) Tail(n int) ([]Event, error) {
	...
	var events []Event
	dec := json.NewDecoder(&byteReader{b: data})
	for dec.More() {
		var e Event
